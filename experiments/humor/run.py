...
Usage:
    python experiments/humor/run.py                    # full run with model
    python experiments/humor/run.py --holdout           # include holdout test
    python experiments/humor/run.py --cached-only       # re-run classifier on cached embeddings (no GPU needed)
    python experiments/humor/run.py --cached-only --holdout  # holdout using cached embeddings
"""
import argparse
import json
import sys
...
def load_stimuli(n: int | None = None) -> pd.DataFrame:
    ...
    if n is not None:
        n_per = n // 2
        humor = df[df.label == POSITIVE_LABEL].head(n_per)
        neutral = df[df.label == NEGATIVE_LABEL].head(n_per)
...
def split_train_holdout(
    ...
    humor = stimuli[stimuli.label == POSITIVE_LABEL]
    neutral = stimuli[stimuli.label == NEGATIVE_LABEL]

    humor_idx = rng.choice(humor.index, size=holdout_per_class, replace=False)
    neutral_idx = rng.choice(neutral.index, size=holdout_per_class, replace=False)
    holdout_idx = np.concatenate([humor_idx, neutral_idx])

    holdout = stimuli.loc[holdout_idx].reset_index(drop=True)
    train = stimuli.drop(holdout_idx).reset_index(drop=True)

    print(f"  Train: {len(train)} ({(train.label == POSITIVE_LABEL).sum()} humor, "
          f"{(train.label == NEGATIVE_LABEL).sum()} neutral)")
    print(f"  Holdout: {len(holdout)} ({(holdout.label == POSITIVE_LABEL).sum()} humor, "
          f"{(holdout.label == NEGATIVE_LABEL).sum()} neutral)")
