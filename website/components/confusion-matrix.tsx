    ...
    <div className="inline-block">
      <div className="flex items-end gap-0">
        <div className="flex flex-col items-end justify-center pr-3 pb-8">
          <p className="text-[10px] uppercase tracking-wider text-[#9C9488] mb-3 -rotate-90 origin-center translate-y-6">
            Actual
          </p>
        </div>
        <div>
          ...
          >
            {data.labels.map((label) => (
              <div key={label} className="flex items-center justify-center w-16 sm:w-20">
                <span className="text-xs text-[#9C9488]">{label}</span>
              </div>
            ))}
          </div>
          <p className="text-[10px] uppercase tracking-wider text-[#9C9488] text-center mt-1">
            Predicted
          </p>
        </div>
      </div>
      <div className="flex items-center justify-end gap-1 mt-3 pr-1">
        {data.labels.map((label, i) => (
          <span key={label} className="text-[10px] text-[#9C9488]">
            {i > 0 && " · "}
            Row {i + 1} = {label}
          </span>
        ))}
